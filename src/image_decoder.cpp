...
ImageDecoder::ImageDecoder(const std::string &input_path) {
...
void ImageDecoder::init_decoder(const std::string &input_path) {
    // Open input file
    int ret = avformat_open_input(&format_ctx_, input_path.c_str(), nullptr, nullptr);
    if (ret < 0) {
        throw std::runtime_error("Failed to open input image file");
    }

    // Find stream information
    ret = avformat_find_stream_info(format_ctx_, nullptr);
    if (ret < 0) {
        throw std::runtime_error("Failed to find image stream info");
    }

    // Find video stream (images are treated as video)
    ...
    if (video_stream_index_ < 0) {
        throw std::runtime_error("No image stream found in file");
    }
    ...
    if (!codec) {
        throw std::runtime_error("Failed to find image decoder");
    }

    // Allocate codec context
    codec_ctx_ = avcodec_alloc_context3(codec);
    if (!codec_ctx_) {
        throw std::runtime_error("Failed to allocate image codec context");
    }

    // Copy codec parameters
    ret = avcodec_parameters_to_context(codec_ctx_, stream->codecpar);
    if (ret < 0) {
        throw std::runtime_error("Failed to copy image codec parameters");
    }
    ...
    if (ret < 0) {
        throw std::runtime_error("Failed to open image codec for decoding");
    }
    ...
    if (!frame_ || !av_packet_) {
        throw std::runtime_error("Failed to allocate frame/packet for image decoding");
    }

    is_gray8_ = (codec_ctx_->pix_fmt == AV_PIX_FMT_GRAY8);

    // Setup conversion if not already gray
    if (!is_gray8_) {
        ...
        if (!sws_ctx_) {
            throw std::runtime_error("Failed to create color conversion context");
        }
    }

    // Setup layout and buffer
    layout_ = compute_image_layout(codec_ctx_->width, codec_ctx_->height);
    extract_buffer_.reserve(static_cast<std::size_t>(layout_.bytes_per_image) * 2);
}

int64_t ImageDecoder::total_images() const {
    if (video_stream_index_ >= 0) {
        const AVStream *stream = format_ctx_->streams[video_stream_index_];
        ...
        if (stream->duration > 0 && stream->time_base.den > 0) {
            ...
            return static_cast<int64_t>(duration_sec);
        }
    }
    return -1;
}

void ImageDecoder::extract_data_into(std::vector<std::byte> &dest) const {
...
#pragma omp parallel for schedule(static)
        for (int byte_idx = 0; byte_idx < total_bytes; ++byte_idx) {
            ...
            for (int sub = 0; sub < 8; ++sub) {
                constexpr int32_t C0 = 8035;
                constexpr int32_t C1 = 6811;
                constexpr int32_t C2 = 4551;
                constexpr int32_t C3 = 1598;
...
std::vector<std::byte> ImageDecoder::extract_data_from_frame() const {
    std::vector<std::byte> data;
    extract_data_into(data);
    return data;
}

std::size_t get_image_packet_size(const std::span<const std::byte> data) {
    if (data.size() < 5) {
...
void ImageDecoder::extract_packets_from_buffer(std::vector<std::byte> &accumulated,
    ...
    while (offset + 4 <= accumulated.size()) {
        ...
        if (magic == MAGIC_ID) {
            const std::size_t pkt_size = get_image_packet_size(
...
std::vector<std::vector<std::byte>> ImageDecoder::extract_packets_from_frame() const {
    const auto raw_data = extract_data_from_frame();
    std::vector<std::vector<std::byte>> packets;
    const std::size_t packet_size = get_image_packet_size(std::span(raw_data));
...
void ImageDecoder::prepare_frame_for_extraction() {
    if (!is_gray8_) {
...
std::vector<std::vector<std::byte>> ImageDecoder::accumulate_frame_and_extract_packets() {
...
std::vector<std::vector<std::byte>> ImageDecoder::flush_decoder_and_collect_packets() {
    ...
    while (true) {
        ...
        if (ret < 0) {
            throw std::runtime_error("Error receiving image frame");
        }
...
std::vector<std::vector<std::byte>> ImageDecoder::decode_next_image() {
    if (eof_) {
        return {};
    }
...
std::vector<std::vector<std::byte>> ImageDecoder::decode_all_images() {
    std::vector<std::vector<std::byte>> results;
    while (!eof_) {
        for (auto packets = decode_next_image(); auto &pkt : packets) {
